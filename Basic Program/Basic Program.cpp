...
void InitScene()
{
	...
	//materials (changing friction coefficient makes it act like its on carpet and not on ice).
	PxMaterial* default_material = physics->createMaterial(0.f, 0.06f, 0.f);   //static friction, dynamic friction, restitution
...
int main()
{
	...
	//get mass and display
	PxReal mass = box->getMass();
	std::cout << "mass of box in Kg: " << mass << endl;

	//set the simulation step to 1/60th of a second
	PxReal delta_time = 1.f/60.0f;
	...
	while (!GetAsyncKeyState(VK_ESCAPE))
	{
		...
		//isSleeping gets returned value from function. if true print that actor is sleeping.
		isSleeping = box->isSleeping();
		if (isSleeping == true)
		{
			std::cout << "actor set to sleep " << endl;
		}
